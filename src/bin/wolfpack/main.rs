...
fn do_main() -> Result<ExitCode, Box<dyn std::error::Error>> {
    ...
    match args.command {
        ...
        Command::Build(more_args) => build_package(config, more_args),
        Command::BuildRepo(more_args) => build_repo(config, more_args),
...
fn build_package(_config: Config, args: BuildArgs) -> Result<ExitCode, Box<dyn std::error::Error>> {
